...
! !ROUTINE: irchar 
! !INTERFACE: 
subroutine  irchar(ik,ngkmax,nmatmax,nstfv,nspnfv, nspinor, & 
                   vgkl, nvec, nsp, symrot, symtrn, symsu2, & 
                   maxdeg,v,ttir,ctir,ztir,ntir,nkgrp,lkgrp, &
                   kgrpr,kgrpt,kgrps,& 
                   kgrpclass,irlab,evalfv,evecfv,toldg) 

!subroutine  irchar( ngkmax,nmatmax,nstfv,nspnfv,nspinor, &
...
!BOC 
implicit none 
integer,      intent(in)  :: ik,ngkmax,nmatmax,nstfv,nspnfv,nspinor
integer,      intent(in)  :: nsp, nvec
real(8),      intent(in)  :: vgkl(3,ngkmax),v(3),toldg 
...
integer       maxdeg,ntir(2),lkgrp(48),nkgrp,kgrpr(3,3,48)
real(8)       kgrpt(3,48)  
...
do ip=1,10
write(*,*) "c ",ik,nkgrp, nvec, (vgkl(is,ip),is=1,3)  
enddo
write(*,*) "d    ", (vgkl(is,ip),is=1,3)
...
write(*,*) "c ",ri(3,1:3)
  do ip=1,nvec 
    ...
    in=1 
    do while(( dabs(vr(1)-vgkl(1,in)) & 
              +dabs(vr(2)-vgkl(2,in)) & 
              +dabs(vr(3)-vgkl(3,in)) ).gt.1d-4) 
      in=in+1 
      if(in.eq.nvec+1)then 
        write(*,*) 
        write(*,'("Error(irchar): no (k+G)inv(R) for ",I5)'),ip 
        write(*,*) 
        stop 
      endif 
    enddo 
...
do while(ie.le.nstfv) 
  ...
  if(ndeg.gt.maxdeg) then 
    write(*,*)  
    write(*,'("Error(irchar): ndeg > maxdeg")') 
        ...
        if(kgrpclass(is)(1:6).eq.ttir(6*(ir-1)+10:6*ir+9)) then 
          ...
          if(iwrt.eq.1) then 
            ...
            if((abs(xi-xr(ir)).gt.1e-6)) then 
              write(*,*)  
              write(*,'("Error(irchar): X(Ri) not X(Rj)")') 
              write(*,*)  
!              stop 
            endif 
          endif 
