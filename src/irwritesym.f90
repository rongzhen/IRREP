...
! !ROUTINE: irwritesym
! !INTERFACE:
subroutine  irwritesym(iout,fl,title,avec,ainv,pgrpname,nsym,symrot, &
...
enddo  
! 
write(iout,'(////"    R     t    axis            R        t   axis",15X,"u(R)")') 
...
!
  write(iout,'(I5,"  class:", A6,8X,$)') is, symcls(is)(1:6)
  cnold(1:6)=symcls(is)(1:6) 
  select case(cnold(2:3))   
    case(' E')           
      write(iout,'(X," unit rotation")') 
    case(' I')          
      write(iout,'(X," inversion")') 
    case('IC') 
      read(cnold(4:4),'(I1)') it 
      if(symaxc(3,is).eq.1) then
        write(iout,'(X,I4,"-degree clockwise rotation, times inversion")') 360/it 
      elseif(symaxc(3,is).eq.1) then
        write(iout,'(X,I4,"-degree rotation, times inversion")') 360/it
      else
        write(iout,'(X,I4,"-degree counterclockwise rotation, times inversion")') 360/it  
      endif 
   case default  
      read(cnold(4:4),'(I1)') it 
      if(symaxc(3,is).eq.1) then
        write(iout,'(X,I4,"-degree clockwise rotation")') 360/it 
      elseif(symaxc(3,is).eq.1) then
        write(iout,'(X,I4,"-degree rotation")') 360/it 
      else
        write(iout,'(X,I4,"-degree counterclockwise rotation")') 360/it  
      endif       
  end select 
enddo 
!
