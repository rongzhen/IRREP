...
! !ROUTINE: irtimerev
! !INTERFACE:
subroutine  irtimerev(v,ztir,ctir,ttir,ntir,cnam,nkgrp,lkgrp,& 
                      nsym,symrot, symtrn, &
...
!    
! !DESCRIPTION: 
!   Checks if the IRs are degenerate due to time reversal symmetry.
! 
! !REVISION HISTORY: 
...
!--------------------------------------------------------------------------! 
trlab(:)=' '
iordk = nint(dble(nsym)/dble(mspro(1))) 
do ir=1,ntir(2) 
!=================================================================OBSSS 
  ircas(ir)=-1000 
  ...
  do ip=1,mspro(2) 
    ...
    if(ip4.eq.0) then  
      write(*,*)  
      write(*,'("Error(irtimerev): not in G(k)")') 
      write(*,*)  
      stop   
    endif 
    icc=0 
...
! 
    do i1=1,3 
      tav(i1)= symrot(i1,1,msneg(ip))*symtrn(1,msneg(ip))& 
              +symrot(i1,2,msneg(ip))*symtrn(2,msneg(ip))& 
              +symrot(i1,3,msneg(ip))*symtrn(3,msneg(ip))+symtrn(i1,msneg(ip)) 
    end do 
  ...
  if(imag(csum).gt.1d-6.or.icc.gt.ntir(1)) then 
    write(*,*) 
    write(*,'("Error(irtimerev): incorrect sum")') 
    write(*,*) 
    stop   
  ...
  elseif(abs(real(csum)+dble(iordk)).lt.1d-6) then 
    ...
    write(*,*)  
    write(*,'("Error(irtimerev): cannot find case (a), (b) or (c)")') 
    write(*,*) mspro(1),mspro(2),iordk,dble(nsym)/dble(mspro(1)),csum,arg 
    write(*,*)  
    stop   
  endif 
end do 
...
! sum over |X_i(Pg)*-X_j(Pg')| 
              iwrt=iwrt+1 
              arg=-twopi*( v(1)*symtrn(1,ip1) & 
                          +v(2)*symtrn(2,ip1) & 
                          +v(3)*symtrn(3,ip1) ) 
              php1=cmplx(cos(arg),sin(arg)) 
! 
              arg=-twopi*( v(1)*symtrn(1,iq1) & 
                          +v(2)*symtrn(2,iq1) & 
                          +v(3)*symtrn(3,iq1) ) 
