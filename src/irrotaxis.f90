...
if(abs(ds-1.0).gt.1e-6) then  
  write(*,*) 
  write(*,'("Error(irrotaxis): not a proper rotation")') 
  write(*,*) 
  stop 
endif 
...
if(ds.gt.1d-6) then 
  ...
  if(it.ge.itmx) then  
    write(*,*) 
    write(*,'("Error(irrotaxis): cannot find angles")') 
    write(*,*) 
    stop 
  endif 
! 
...
! all but for 0 and 180 degree rotations 
  ...
  if(ds.gt.1d-6) then 
    ...
    elseif(dmin.lt.-1e-6) then 
      ...
      write(*,*) 
      write(*,'("Error(irrotaxis): cannot find rotation direction")') 
      write(*,*)  
!      stop 
    endif 
  endif 
