...
! of the single-electron eigenfunction. This symmetry is a fundamental physical quantity of the 
...
! but also when developing and implementing new perturbation Hamitonians and/or implementing 
! new observables into the computer codes. 
...
! saves disc space for systems with large crystal unit cells.  
!
! Please note that
! \begin{itemize}
!   \item The notation of the IRs is according to  
!         G.F. Koster, J.O. Dimmock, R.G. Wheeler, H. Statz, 
!        "Properties of the Thirty-Two Point Groups" (MIT Press, Cambridge MA, 1963).
...
! Below, a very brief introduction to IRs is given, following the discussion by 
! J. F. Cornwell, "Group Theory in Physics", Vol. 1 (Academic Press, London, 1984).
! A somewhat more complete description of the theories and also a description of 
! the implementatin can be found in 
! C. Persson, Comput. Phys. Commun. {\bf XX}, XX (2008), wherein also properties of the 
! IRs are illustrated and references are given. 
!
! \subsection{Crystallographic space groups}
! The set of all coordinate transformations ${\bf P}_g$ ($g$ = 1, 2, ... $\infty$) that maps 
! the atomicpositions in an infinite crystal into itself forms an infinite group 
! $\mathcal{G}_{\infty}$, called the {\it crystallographic space group}. 
! An element ${\bf P}_g$ = $\{{\bf R}_g|{\bf t}_g\}$ of the space group consists of a
! rotation part ${\bf R}_g$ and a translation part ${\bf t}_g$. 
! By definition, the coordinate transformations leave the Hamiltonian invariant, i.e.,
! ${\bf P}_g$ and $\hat{H}$ commute.
! The inverse transformation ${\bf P}_g^{-1}$  = $\{{\bf R}_g^{-1}|-{\bf R}_g^{-1}{\bf t}_g\}$.
! Operating a coordinate transformation $\{{\bf R}_g|{\bf t}_g\}$ $\in$ $\mathcal{G}_{\infty}$ 
...
! To every coordinate transformation $\{{\bf R}_g|{\bf 0}\}$ of $\mathcal{G}_0$ there 
...
! where $n_\iota$ are integer numbers ($\iota$ = 1, 2, and 3) and ${\bf a}_\iota$ are the 
...
! The corresponding space group $\mathcal{G}$, containing the finite number of
! transformations $\{{\bf R}_g|{\bf \tau}_g+{\bf t}_n\}$, has the same point group
...
! Let the $d$ linearly independent functions $\psi_1({\bf r})$, 
! $\psi_2({\bf r})$, ... $\psi_d({\bf r})$, span the particle space $W$. 
! If all group elements  ${\bf P}_g$ of $\mathcal{G}$ operating on $\psi_i({\bf r})$ 
...
! Each element ${\bf P}_g$ of the space group $\mathcal{G}$ can be assigned to a non-singular 
...
! of basis functions. However, the trace of a representation matrix, called the {\it character} of 
...
! $\Gamma^{\mathcal{G}}_{\alpha}({\bf P}_g)$ for all ${\bf P}_g$, forms 
!  the $d_{\alpha}$-dimensional {\it irreducible representation} (IR) $\Gamma^{\mathcal{G}}_{\alpha}$ of 
! the space group $\mathcal{G}$. 
...
! $\oplus$ is defined by the block-diagonal form of the representation matrices 
! $\Gamma^{\mathcal{G}}({\bf P}_g)$, as shown in Eq.~(\ref{e:G_blockAB}).
...
! 
! \begin{equation}\label{e:G_dirprod}
...
! Here it is sufficient to use two-component spinors which are used in the present code:
! 
...
! operating on a spinor within the real space, also a transformation ${\bf U}({\bf R}_g^p)$ is 
! operating on spin components within the spin-space
! 
! \begin{eqnarray}\label{e:DG_operate}
!  {\bf U}({\bf R}_g^p){\bf P}_g\ \bar{\psi}_{j_{\alpha}{\bf k}}({\bf r}) =
!  \bar{\psi}'_{j_{\alpha}{\bf k}}({\bf r}') = 
!  \pm\sum_{j'} \Gamma^{\mathcal{G}^D}_{\alpha}({\bf P}_g)_{jj'}
...
! \end{eqnarray}
! 
! where ${\bf R}_g^p$ is the {\it proper rotation} of ${\bf R}_g$ [i.e, $|{\bf R}_g|$ = 1 ].
! The sign for $\pm$ is entirely a matter of convention, but it 
...
! through the Eulers angles $\varphi$, $\theta$, and $\xi$ as:
!
...
! \end{equation}
! 
! $\Gamma^{\mathcal{G}_0({\bf k})}_{\alpha}$ is called the {\it relevant} IR of $\mathcal{G}({\bf k})$.
! This is a very useful relation between the space-group IR and the point-group IR, and commonly used when 
! presenting the IRs in electronic band structures
! The relation of Eq.~\ref{e:Allowed2} holds for all symmorphic space groups, and 
! also for every internal point of the Brillouin zone. However, it can (but not have to)
...
! not comprised in the set of coordinate transformations and thereby needed to be considered
! additionally by the time-reversal operator $\hat{K}$.
! For spin-independent systems $\hat{K}$ = $\hat{K}_0$, where $\hat{K}_0$ is defined by 
! $\hat{K}_0\bar{\psi}_{i}({\bf r})$ = $\bar{\psi}^*_{i}({\bf r})$
