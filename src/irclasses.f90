...
! !ROUTINE: irclasses
! !INTERFACE: 
subroutine  irclasses(ns,lr,r,nc,cn) 
!
...
! determine it, where (R)^i=E 
  ...
  do it=1,ns 
    ...
    call irfindmmi(roi,dble(r(1:3,1:3,lr(1:ns))),ns,n,ds) 
    if(ds.lt.1d-6) then 
      iopjij(is,it)=n 
    else 
      write(*,*) 
      write(*,'("Error(irclasses): no Pn=Pj*Pi*inv(Rj)")') 
      write(*,*) 
      stop 
    end if 
  end do 
...
do is=1,ns 
  ...
  case default 
    write(*,*) 
    write(*,'("Error(irclasses): cannot find class name")') 
    write(*,*) 
    stop 
  end select 
enddo  
